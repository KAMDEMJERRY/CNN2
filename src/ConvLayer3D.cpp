...
Tensor ConvLayer3D::forward(const Tensor& input) {
    ...
    if (od.d <= 0 || od.h <= 0 || od.w <= 0)
        throw std::runtime_error("[ConvLayer3D] forward: dimensions de sortie nulles ou négatives");
...
Tensor ConvLayer3D::backward(const Tensor& gradOutput) {
    int B    = input_cache.dim(0);
    int D_in = input_cache.dim(2);
    int H_in = input_cache.dim(3);
    int W_in = input_cache.dim(4);
    ...
    // 4. Gradient vers l'entrée
    Eigen::MatrixXf W_mat;
    convertWeightsToMatrix(W_mat);
    Eigen::MatrixXf dCol = W_mat.transpose() * dO;
